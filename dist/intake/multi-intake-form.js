...
import { CustomerContactFields, } from "./customer-select.js";
...
export default function MultiIntakeForm({ action, fields, customerRefColumn, contactRefColumn, accountEndpoint, contactEndpoint, cancelHref, unitLabel, submitLabel, }) {
    ...
    const errorFieldsFor = (index) => new Set(errors.filter((e) => e.index === index).map((e) => e.field).filter(Boolean));
    const setUnit = (i, col, v) => setUnits((prev) => prev.map((u, j) => (j === i ? { ...u, [col]: v } : u)));
    const addUnit = () => setUnits((prev) => [...prev, {}]);
    const removeUnit = (i) => setUnits((prev) => (prev.length === 1 ? prev : prev.filter((_, j) => j !== i)));
